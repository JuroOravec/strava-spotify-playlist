...
const createPlaylistSpotifyServices = (): PlaylistSpotifyServices => {
  const getRecentlyPlayedTracks: PlaylistSpotifyServices['getRecentlyPlayedTracks'] = async function getRecentlyPlayedTracks(
    this: ThisModule,
    input
  ) {
  ...
  async function enrichTracks(
    ...
    const tracksMetadata = await batchedGetTracks({
      ...
      trackIds: tracks
        .filter(
          (track) => track.playlistProviderId === PlaylistProvider.SPOTIFY
        )
...
export default createPlaylistSpotifyServices;
export type { PlaylistSpotifyServices };
